 ...
 *      - onError: 可选参数，一个处理错误的回调函数，接受一个 result 类型的参数。
 *                 Optional parameter, a callback function for handling errors, accepting an result type parameter.
 *      - actionParams: 可选参数，一个数组，包含传递给 action 函数的参数。
 ...
 *  - error: result 实例，如果异步操作执行中发生错误，则此变量会被设置；否则为 null。
 *           result instance, set if an error occurs during the asynchronous operation; otherwise null.
 *
 * 使用示例 (Usage Example):
 * ```
...
const useAsyncHandler = <T extends any>(): [
    (action: AsyncHandlerFunc<T | null | undefined>,
     actionParams: any[],
     onError?: AsyncHandlerErrorFunc, onCleanup?: AsyncHandlerVoidFunc
    ) => Promise<T | null>, boolean, T | null,
] => {
    ...
    const run: useAsyncHandlerCallbackProps<T> = useCallback(async (
        ...
        {
            throw new Error("userAsyncHandler Error: The action parameter must be a function when provided.");
        }
