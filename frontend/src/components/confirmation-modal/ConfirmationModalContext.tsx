...
interface ConfirmationModalContextType {
    showConfirmation: <T>(props: Omit<ConfirmationModalProps<T>, 'isOpen' | 'onClose'>) => void
...
export const ConfirmationModalProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    // 泛型状态，默认支持 any
    const [modalProps, setModalProps] = useState<ConfirmationModalProps<any> | null>(null)

    // showConfirmation 支持泛型，用于动态设置 ifValue 的类型
    const showConfirmation = <T,>(props: Omit<ConfirmationModalProps<T>, 'isOpen' | 'onClose'>) => {
        setModalProps({
            ...props,
            isOpen: true,
